...
# TODO: Figure out how to extend the `vagrant ssh` command to add the ssh identify if necessary
# require 'net/ssh'
...
#         identityFound = agent.identities.any? { |id| id.comment.match(/id_d|rsa/) }
#         if not identityFound
#           # If we are still running, no identity was found. Add one
#           @env.ui.info "No Identity found, running ssh-add to add one."
